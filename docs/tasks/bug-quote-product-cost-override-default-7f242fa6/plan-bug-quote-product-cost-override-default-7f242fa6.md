...
## Solution Statement
Add a new private helper function `setDefaultCostOverride(formContext)` that reads the `dc_costoverride` attribute and, only when its current value is `null`/`undefined`, sets it to `5`. Call this helper from `formOnLoad` — after `setDefaultCostOverride` runs, re-order the call to `setCostOverrideDescriptionRequired` so it fires last, ensuring `dc_overridedescription` is correctly marked `required` because `dc_costoverride` is now non-null on new records.
...
- **`dc_quote_product_main_operations.js` — lines 213–216** (`SetMarginsAndDefaults`): the established guard pattern for "only set when empty":
  ...
  ```
- **`dc_quote_product_main_operations.js` — lines 467–485** (`setCostOverrideDescriptionRequired`): shows how `dc_costoverride` is accessed via `formContext.getAttribute("dc_costoverride")`.
...
- In the `formOnLoad` function (lines 5–12), add the call to `setDefaultCostOverride(formContext)` **before** `setCostOverrideDescriptionRequired` so the description field's required-level check sees the newly-set default value:
  ```js
  ...
  ```
- Moving `setCostOverrideDescriptionRequired` to after `setDefaultCostOverride` ensures that on a new record, `dc_overridedescription` is immediately marked `required` (because `dc_costoverride` is now `5`, non-null) rather than left as `none`.
...
- Confirm that `setCostOverrideDescriptionRequired` still functions correctly:
  - On a **new** record (no prior value): `setDefaultCostOverride` sets `dc_costoverride = 5`, then `setCostOverrideDescriptionRequired` marks `dc_overridedescription` as `required`.
  - On an **existing** record with a non-null `dc_costoverride`: `setDefaultCostOverride` skips (guard prevents overwrite), `setCostOverrideDescriptionRequired` marks `dc_overridedescription` as `required`.
  - On an **existing** record where `dc_costoverride` is null: `setDefaultCostOverride` sets it to `5`, `setCostOverrideDescriptionRequired` marks `dc_overridedescription` as `required`.
...
- Open an **existing** Quote Product record that already has a non-null `dc_costoverride` (e.g., `10`): confirm the existing value is **not** overwritten.
...
- Confirm all other `formOnLoad` behaviours continue working (lock logic, margin defaults, product filter).
...
- `dc_overridedescription` is correctly marked as `required` on form load for new records and existing records with a null `dc_costoverride` (because `dc_costoverride` is now `5`, non-null, after the default is applied).
...
- The fix uses `var` for the attribute variable to match the legacy `var`-style used inside `setCostOverrideDescriptionRequired` and other functions in this file; do not introduce `const`/`let` into private functions that already use `var` unless refactoring the whole file.
- An identical feature plan exists at `docs/tasks/feature-set-dc-costoverride-on-load-e0a95fe3/` and covers the same change — this bug plan supersedes it and makes the issue classification explicit.
