...
(function () {
    ...
    this.formOnLoad = function (executionContext) {
        ...
        showHideOverridePriceByQuoteState(formContext);
    }
    ...
    // Show or hide dc_overrideprice based on the related quote's statecode.
    // statecode 3 = Won — hide the field. Otherwise show it.
    async function showHideOverridePriceByQuoteState(formContext) {
        const quoteAttr = formContext.getAttribute("quoteid");
        if (!quoteAttr || !quoteAttr.getValue() || quoteAttr.getValue().length === 0) return;

        const relatedQuoteId = quoteAttr.getValue()[0].id.slice(1, -1);
        ...
        try {
            ...
            console.error("Navco: Failed to retrieve quote statecode for dc_overrideprice visibility.", error.message);
        }
    }
