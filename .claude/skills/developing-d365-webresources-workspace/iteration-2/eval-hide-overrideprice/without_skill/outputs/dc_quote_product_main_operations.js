...
(function () {
    ...
    // Hide dc_overrideprice when the related quote is Won (statecode = 3), show it otherwise
    async function toggleOverridePriceVisibility(formContext) {
        ...
        const relatedQuoteId = quoteAttr.getValue()[0].id.replace("{", "").replace("}", "");

        try {
            const result = await Xrm.WebApi.retrieveRecord("quote", relatedQuoteId, "?$select=statecode");

            const control = formContext.getControl("dc_overrideprice");
            if (!control) return;
