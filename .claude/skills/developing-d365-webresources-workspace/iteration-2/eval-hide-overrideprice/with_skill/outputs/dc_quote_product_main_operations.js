...
(function () {
    ...
    // Hide dc_overrideprice when the related quote is Won (statecode === 3), show otherwise
    async function toggleOverridePriceVisibility(formContext) {

        const quoteAttr = formContext.getAttribute("quoteid") || formContext.getAttribute("dc_quoteid");

        if (!quoteAttr || !quoteAttr.getValue() || !quoteAttr.getValue().length) return;

        const quoteId = quoteAttr.getValue()[0].id.slice(1, -1);

        try {
            const result = await Xrm.WebApi.retrieveRecord("quote", quoteId, "?$select=statecode");
            const isWon = result.statecode === 3;
            formContext.getControl("dc_overrideprice")?.setVisible(!isWon);
        } catch (error) {
            console.error("Navco: Error retrieving quote statecode for dc_overrideprice visibility.", error.message);
        }
    }
